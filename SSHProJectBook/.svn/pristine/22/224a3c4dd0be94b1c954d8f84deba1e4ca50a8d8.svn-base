...
<BODY>
	<FORM id="customerForm" name="customerForm" action="${pageContext.request.contextPath }/money_findByPage.action" method=post>
				...
				<TR>
					...
					<TD vAlign=top width="100%" bgColor=#ffffff>
						<TABLE cellSpacing=0 cellPadding=5 width="100%" border=0>
							<TR>
								<TD class=manageHead>当前位置：罚款记录管理 &gt; 罚款记录列表</TD>
							</TR>
							<TR>
								<TD height=2></TD>
							</TR>
									...
									<TD>
										<TABLE id=grid
											...
											<TBODY>
												...
												<c:forEach items="${page.beanList }" var="money">
													<TR
														style="FONT-WEIGHT: normal; FONT-STYLE: normal; BACKGROUND-COLOR: white; TEXT-DECORATION: none">
														<TD>${money.mid }</TD>
														<TD>${money.reader.sid }</TD>
														<TD>${money.book.bid }</TD>
														<TD>${money.fine_time }</TD>
														<TD>${money.sum }</TD>
														<TD>${money.state }</TD>
														<TD>${money.pay_time }</TD>
														<TD>
														<a href="${pageContext.request.contextPath }/money_initUpdate.action?mid=${money.mid}">修改</a>
														&nbsp;&nbsp; 
